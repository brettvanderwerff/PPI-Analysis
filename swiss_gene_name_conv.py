...
def run(id_converted_df):
    ...
    gene names.
    '''
    gene_names_A = find_gene_name(id_converted_df=id_converted_df, label='Parsed A ID')
    gene_names_B = find_gene_name(id_converted_df=id_converted_df, label='Parsed B ID')
    return pd.concat([id_converted_df, gene_names_A, gene_names_B], axis=1)
...
if __name__ == '__main__':
    ...
    run(id_converted_df=id_converted_df)
